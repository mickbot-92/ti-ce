...
{
    if (memmgr_initialized<0) memmgr_init();
    mem_header_t* p;
    mem_header_t* prevp;
...
void * nrealloc(void *ptr, size_t size){
  void * dest=tmalloc(size);
  if (!dest || !ptr)
    return dest;
#if 0 // don't know where the size of the block is
  mem_header_t* block;
  block = ((mem_header_t*) ptr) - 1;
  size_t S=sizeof(mem_header_t)*block->s.size;
  printf("realloc %d %d",size,S);
  //if (S<size) size=S;
#endif
  // copy (maybe too much)
